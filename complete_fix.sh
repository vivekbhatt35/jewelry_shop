...
# Complete fix for alert issues
echo "Applying complete fix for alert issues..."

# 1. Create a fixed detection_analysis.py file
echo "Creating fixed detection_analysis.py file..."
...
# 2. Stop the alert-logic container
echo "Stopping the alert-logic container..."
docker-compose stop alert-logic

# 3. Copy the fixed file to the container
echo "Copying the fixed file to the container..."
# First make sure the container exists even if it's stopped
CONTAINER_ID=$(docker ps -a --filter "name=yolo_pose_api-alert-logic" -q | head -n 1)

if [ -n "$CONTAINER_ID" ]; then
    echo "Found container: $CONTAINER_ID"
    docker cp detection_analysis.py $CONTAINER_ID:/app/alert_logic/logic/
    echo "File copied successfully!"
else
    echo "Alert logic container not found. Creating it..."
...
# 4. Start the alert-logic container
echo "Starting the alert-logic container..."
docker-compose start alert-logic

# 5. Verify the fix
echo "Verifying logs for alert-logic..."
sleep 2
docker-compose logs --tail=20 alert-logic

echo ""
echo "Fix applied! The system should now generate alerts for masks, helmets, and suspicious objects."
