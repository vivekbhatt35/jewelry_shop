...
echo "Step 1: Creating a script to run inside the container..."
cat << 'EOF' > test_inside_container.py
#!/usr/bin/env python3
import os
import json
import cv2
...
echo "Step 2: Copying the script to the container and running it..."
docker cp test_inside_container.py yolo_pose_api-alert-logic-1:/app/
docker exec -it yolo_pose_api-alert-logic-1 python3 /app/test_inside_container.py

echo ""
echo "Step 3: Checking for alert images with the new naming format..."
